...
async function getUserFromToken(env, accessToken) {
  ...
  if (!res.ok) {
    console.error('[resolveSession] getUserFromToken failed', res.status, await res.text());
    return null;
  }
  return res.json();
}
...
async function getMyTenants(env, accessToken) {
  ...
  if (!res.ok) {
    console.error('[resolveSession] getMyTenants failed', res.status, await res.text());
    return [];
  }
  const rows = await res.json();
  console.log('[resolveSession] getMyTenants rows:', rows.length);
