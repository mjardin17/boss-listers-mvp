// POST /api/channels/tiktok-shop/disconnect
// Removes THIS caller's tenant's stored TikTok Shop OAuth connection via
// the disconnect_marketplace_connection RPC (migration 0020). Mirrors
...
export default async function handler(req, res) {
  ...
  try {
    const rpcRes = await fetch(`${process.env.SUPABASE_URL}/rest/v1/rpc/disconnect_marketplace_connection`, {
      ...
      body: JSON.stringify({ p_marketplace: "tiktok-shop" }),
    });
    if (!rpcRes.ok) {
