...
// Must match the key used in pages/channels.js's startShopifyConnect().
...
// (?code=...&shop=...&state=... in the query string). This is a real page,
// not just an API route — mirrors pages/channels/amazon-callback.js exactly,
// for the same reason: it needs the customer's own Supabase session from
// localStorage via authedFetch.
export default function ShopifyCallback() {
  ...
  useEffect(() => {
    ...
    const { code, state, shop, error: oauthError } = router.query;

    if (oauthError) {
      setStatus("error");
      setMessage(`Shopify declined the connection: ${oauthError}`);
      return;
    }
    if (!code) {
      setStatus("error");
      setMessage("No authorization code came back from Shopify.");
      return;
    }
    if (!shop) {
      setStatus("error");
      setMessage("No store domain came back from Shopify.");
      return;
    }

    const expectedState = sessionStorage.getItem(SHOPIFY_STATE_STORAGE_KEY);
    sessionStorage.removeItem(SHOPIFY_STATE_STORAGE_KEY);
    ...
    authedFetch("/api/channels/shopify/callback", {
      method: "POST",
      ...
      .then(async (res) => {
        ...
        setMessage(
          data.accountIdentifier
            ? `Connected to Shopify as ${data.accountIdentifier}.`
  ...
  return (
    <div className="app-shell" style={{ maxWidth: 480, paddingTop: "4rem", textAlign: "center" }}>
      <h1>Shopify Store Connection</h1>
