  ...
  try {
    ...
    if (data.error) {
      return res.status(400).send(`
        <html>
          <head><title>YouTube Authorization Error</title><style>
    ...
    const saved = await saveOAuthToken('youtube', {
    ...
    if (!saved.success) {
      console.error('Failed to save token:', saved.error)
    }

    return res.send(`
      <html>
        <head><title>YouTube Authorization Success</title><style>
          body{font-family:system-ui;padding:40px;max-width:600px;margin:0 auto}
        ...
        <body>
          ...
          <div class="success">
            <p><strong>Your YouTube account is now connected to BossLister</strong></p>
          ...
          <div class="info">
            ...
            <ul>
              ...
              <li>You can now post videos to YouTube</li>
            </ul>
          </div>
