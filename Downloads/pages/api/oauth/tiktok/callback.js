  ...
  try {
    ...
    if (data.error) {
      return res.status(400).send(`
        <html>
          <head><title>TikTok Authorization Error</title><style>
    ...
    const saved = await saveOAuthToken('tiktok', {
    ...
    if (!saved.success) {
      console.error('Failed to save TikTok token:', saved.error)
    }

    return res.send(`
      <html>
        <head><title>TikTok Authorization Success</title><style>
          body{font-family:system-ui;padding:40px;max-width:600px;margin:0 auto}
        ...
        <body>
          ...
          <div class="success">
            <p><strong>Your TikTok account is now connected to BossLister</strong></p>
          ...
          <div class="info">
            ...
            <ul>
              ...
              <li>You can now post videos to TikTok</li>
            </ul>
          </div>
          <h3>Access Token (for reference):</h3>
          <div class="token">${data.data.access_token}</div>
          <button onclick="navigator.clipboard.writeText('${data.data.access_token}')">📋 Copy Token</button>
          <p><a href="/get-tokens">← Back to OAuth Tests</a></p>
        </body>
      </html>
    `)
