  ...
  try {
    ...
    if (data.error) {
      return res.status(400).send(`
        <html>
          <head><title>Instagram Authorization Error</title><style>
    ...
    const saved = await saveOAuthToken('instagram', {
      access_token: data.access_token,
      expires_in: data.expires_in,
    })

    if (!saved.success) {
      console.error('Failed to save Instagram token:', saved.error)
    }

    return res.send(`
      <html>
        <head><title>Instagram Authorization Success</title><style>
          body{font-family:system-ui;padding:40px;max-width:600px;margin:0 auto}
        ...
        <body>
          ...
          <div class="success">
            <p><strong>Your Instagram account is now connected to BossLister</strong></p>
          ...
          <div class="info">
            ...
            <ul>
              ...
              <li>You can now post to Instagram</li>
            </ul>
          </div>
