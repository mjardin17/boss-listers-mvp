  ...
  try {
    const response = await fetch('https://api.amazon.com/auth/o2/token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/oauth/amazon/callback`,
      }),
    })
    ...
    if (data.error) {
      return res.status(400).send(`
        <html>
          <head><title>Amazon Authorization Error</title><style>
    ...
    const saved = await saveOAuthToken('amazon', {
    ...
    if (!saved.success) {
      console.error('Failed to save Amazon token:', saved.error)
    }

    return res.send(`
      <html>
        <head><title>Amazon Authorization Success</title><style>
          body{font-family:system-ui;padding:40px;max-width:600px;margin:0 auto}
        ...
        <body>
          <h1>✅ Amazon Authorization Successful!</h1>
          <div class="success">
            <p><strong>Your Amazon account is now connected to BossLister</strong></p>
          ...
          <div class="info">
            ...
            <ul>
              ...
              <li>You can now manage Amazon Selling Partner API</li>
            </ul>
          </div>
