...
import { publishToInventory } from "./supabaseInventory";
import { generateInternalSku } from "./inventoryEngine/skuGenerator";
import { ensurePublicBucketExists, uploadPublicImage } from "./supabaseStorage";
...
async function persistUploads(files) {
  ...
  return Promise.all(
    files.map(async (file) => {
      ...
      // Local disk write is kept — analyzeProductImages() (openaiVision.js)
      // reads this real filepath to send bytes to OpenAI's vision API, and
      // this repo's local dev server also serves /public statically for
      // in-app previews.
      await fs.writeFile(filepath, buffer);

      // ALSO upload to Supabase Storage (public bucket) to get a real
      // https:// URL. A local disk path or a LAN address (e.g.
      // http://10.0.0.33:3001/uploads/...) is not fetchable by eBay's (or
      // any marketplace's) own servers, which pull product images by URL —
      // and on Render, local disk is wiped on every redeploy anyway, so
      // this is also what makes photos survive a deploy.
      let publicUrl = null;
      try {
        await ensurePublicBucketExists();
        publicUrl = await uploadPublicImage(buffer, filename, file.type || "image/jpeg");
      } catch (error) {
        logWarn("upload.public_storage_failed", {
          error: error?.message || "Failed to upload image to Supabase Storage; falling back to local-only URL."
        });
      }
      ...
      return {
        ...
        relativeUrl: publicUrl || `/${uploadsDir}/${filename}`,
        localUrl: `/${uploadsDir}/${filename}`,
        publicUrl,
...
export async function analyzeFormData(formData, context = {}) {
  ...
  // Also publish into the REAL shared inventory (Supabase `products` table)
  // that the Inventory page and dashboard actually read from. saveListing()
  // above only writes to a local JSON file nothing else in the app reads —
  // without this, a generated listing would never show up anywhere else in
  // the app, and on Render that local file is wiped on every redeploy
  // anyway. Best-effort: a failure here must not break listing generation.
  let inventorySku = null;
  if (shouldGenerate && primaryDraft) {
    try {
      inventorySku = generateInternalSku({
        brand: analysis.brand || input.brand || "",
        title: primaryDraft.title || analysis.itemTitle || "",
        upc: analysis.upc || input.upc || ""
      });
      await publishToInventory({
        sku: inventorySku,
        title: primaryDraft.title || analysis.itemTitle || "Untitled item",
        description: primaryDraft.description || "",
        price: Number(pricing.recommendedPrice) || Number(pricing.selectedPrice) || 0.01,
        quantity: Number(input.quantity || analysis.quantity) || 1,
        imageUrl: imageUrls[0] || null,
        condition: analysis.condition || input.condition || null
      });
      logInfo("scan.inventory.published", { requestId, sku: inventorySku });
    } catch (error) {
      logWarn("scan.inventory.publish_failed", {
        requestId,
        userId: context.userId || "anonymous",
        error: error?.message || "Failed to publish generated listing to shared inventory."
      });
    }
  }
