...
class EbayConnector extends BaseConnector {
  ...
  /** Same live probe as testConnection(), but against THIS tenant's own
   * stored OAuth connection instead of the shared app-level
   * EBAY_REFRESH_TOKEN — testConnection() alone can never prove a specific
   * customer's eBay account still works, only that the app's own shared
   * credential does. Used by the Connected Accounts page. Never called by
   * testConnection() itself, so the already-live shared-credential path is
   * untouched. */
  async testTenantConnection(tenantId) {
    if (!tenantId) return { status: CONNECTION_STATUS.NOT_CONNECTED, detail: "No tenant to check." };
    let accessToken;
    try {
      accessToken = await this._getAccessToken(tenantId);
    } catch (err) {
      const hint = err.ebayError === "invalid_grant"
        ? " Reconnect eBay to mint a new token."
        : "";
      return { status: CONNECTION_STATUS.CONFIG_REQUIRED, detail: `${err.message}.${hint}` };
    }
    try {
      ...
      return {
        status: CONNECTION_STATUS.CONNECTED,
        detail: `Your eBay account is connected (${count} fulfillment ${count === 1 ? "policy" : "policies"} on EBAY_US)`,
      };
    } catch (err) {
      return {
        status: CONNECTION_STATUS.CONFIG_REQUIRED,
...
class EtsyConnector extends BaseConnector {
  ...
  /** Live probe against THIS tenant's own connected shop, not just the
   * app-level key testConnection() checks. Fetches the shop record by the
   * shop_id stored at connect time — proves the specific stored access
   * token still works for the specific shop, not merely that the app's
   * ETSY_KEYSTRING is valid. Used by the Connected Accounts page. */
  async testTenantConnection(tenantId) {
    if (!tenantId) return { status: CONNECTION_STATUS.NOT_CONNECTED, detail: "No tenant to check." };
    let cached;
    try {
      cached = await this._getAccessToken(tenantId);
    } catch (err) {
      return { status: CONNECTION_STATUS.CONFIG_REQUIRED, detail: err.message };
    }
    try {
      const res = await fetch(`${EtsyConnector.API_BASE}/shops/${cached.shopId}`, {
        headers: {
      ...
      if (!res.ok) {
        return { status: CONNECTION_STATUS.CONFIG_REQUIRED, detail: `Token stored, but Etsy rejected it (HTTP ${res.status}) — reconnect Etsy.` };
      }
      const shop = await res.json().catch(() => null);
      return {
        status: CONNECTION_STATUS.CONNECTED,
        detail: `Your Etsy shop is connected${shop?.shop_name ? ` ("${shop.shop_name}")` : ""}.`,
      };
    } catch (err) {
      return {
        status: CONNECTION_STATUS.CONFIG_REQUIRED,
...
class AmazonConnector extends BaseConnector {
  ...
  /** The only live check that can ever prove Amazon "connected" — SP-API
   * has no app-level ping (see testConnection() above), so this tenant
   * probe IS the real status check the Connected Accounts page needs.
   * marketplaceParticipations is read-only and lists which marketplaces
   * this seller account participates in — a lightweight, side-effect-free
   * way to prove the stored access token is live. */
  async testTenantConnection(tenantId) {
    if (!tenantId) return { status: CONNECTION_STATUS.NOT_CONNECTED, detail: "No tenant to check." };
    let cached;
    try {
    ...
    try {
      const res = await fetch("https://sellingpartnerapi-na.amazon.com/sellers/v1/marketplaceParticipations", {
        headers: { "x-amz-access-token": cached.token, Accept: "application/json" },
        signal: AbortSignal.timeout(10_000),
      });
      const body = await res.json().catch(() => null);
      if (!res.ok) {
        ...
        return {
          status: CONNECTION_STATUS.CONFIG_REQUIRED,
          detail: `Token stored, but SP-API returned HTTP ${res.status}${e ? ` (${e.code}: ${e.message})` : ""} — reconnect Amazon.`,
        };
      }
      const count = body?.payload?.length ?? 0;
      return {
        status: CONNECTION_STATUS.CONNECTED,
        detail: `Your Amazon Seller Central account is connected (${count} marketplace ${count === 1 ? "participation" : "participations"}).`,
      };
    } catch (err) {
      return {
        status: CONNECTION_STATUS.CONFIG_REQUIRED,
        detail: err.name === "TimeoutError" ? "Amazon SP-API did not respond within 10s" : `Amazon SP-API request failed: ${err.message}`,
      };
    }
  }
...
class TikTokShopConnector extends BaseConnector {
  ...
  /** The only live check that can ever prove TikTok Shop "connected" — no
   * app-level ping exists (see testConnection() above). Calls the standard
   * "Get Authorized Shops" endpoint, which lists the shops this specific
   * access token is authorized for — read-only, no side effects. */
  async testTenantConnection(tenantId) {
    if (!tenantId) return { status: CONNECTION_STATUS.NOT_CONNECTED, detail: "No tenant to check." };
    let cached;
    try {
    ...
    try {
      ...
      const query = TikTokShopConnector._signedQuery(pathname, {});
      const res = await fetch(`https://open-api.tiktokglobalshop.com${pathname}?${query.toString()}`, {
        headers: { "x-tts-access-token": cached.token },
        signal: AbortSignal.timeout(10_000),
      });
      const body = await res.json().catch(() => null);
      if (!res.ok || !body || body.code !== 0) {
        return {
          status: CONNECTION_STATUS.CONFIG_REQUIRED,
          detail: `Token stored, but TikTok Shop rejected it${body?.message ? ` (${body.message})` : ` (HTTP ${res.status})`} — reconnect TikTok Shop.`,
        };
      }
      const count = body.data?.shops?.length ?? 0;
      return {
        status: CONNECTION_STATUS.CONNECTED,
        detail: `Your TikTok Shop is connected (${count} authorized ${count === 1 ? "shop" : "shops"}).`,
      };
    } catch (err) {
      return {
        status: CONNECTION_STATUS.CONFIG_REQUIRED,
        detail: err.name === "TimeoutError" ? "TikTok Shop did not respond within 10s" : `TikTok Shop request failed: ${err.message}`,
      };
    }
  }
...
class ShopifyConnector extends BaseConnector {
  ...
  /** Live probe against THIS tenant's own connection — via
   * _getAccessToken(tenantId), which already resolves (in order) a cached
   * token, the tenant's own OAuth-connected store, or the single-store
   * static SHOPIFY_ADMIN_TOKEN fallback. Covers the "which one exists
   * today" question directly: the `source` field on the cached token says
   * whether this tenant is on tenant_oauth or still riding the shared
   * static_admin_token path. Used by the Connected Accounts page. */
  async testTenantConnection(tenantId) {
    let cached;
    try {
      cached = await this._getAccessToken(tenantId);
    } catch (err) {
      return { status: CONNECTION_STATUS.NOT_CONNECTED, detail: err.message };
    }
    const origin = cached.source === "tenant_oauth"
      ? `https://${cached.accountIdentifier}`
      : (() => { try { return ShopifyConnector.normalizeStoreOrigin(process.env.SHOPIFY_STORE_URL); } catch (err) { return null; } })();
    if (!origin) {
      return { status: CONNECTION_STATUS.CONFIG_REQUIRED, detail: "Shopify token found, but no store domain to call it against." };
    }
    try {
      const res = await fetch(`${origin}/admin/api/2024-10/shop.json`, {
        headers: { "X-Shopify-Access-Token": cached.token },
        signal: AbortSignal.timeout(10_000),
      });
      const body = await res.json().catch(() => null);
      if (!res.ok) {
        return { status: CONNECTION_STATUS.CONFIG_REQUIRED, detail: `Token stored, but Shopify rejected it (HTTP ${res.status}) — reconnect Shopify.` };
      }
      return {
        status: CONNECTION_STATUS.CONNECTED,
        detail: `Your Shopify store is connected ("${body?.shop?.name || origin}", via ${cached.source === "tenant_oauth" ? "your own OAuth connection" : "the shared store admin token"}).`,
...
const { SnapchatShopConnector, SnapchatShopListingError } = require("./SnapchatShopConnector");
const { TwitterCommerceConnector, TwitterCommerceListingError } = require("./TwitterCommerceConnector");
const { PinterestShopConnector, PinterestShopListingError } = require("./PinterestShopConnector");
...
module.exports = {
  ...
  SnapchatShopConnector, SnapchatShopListingError,
  TwitterCommerceConnector, TwitterCommerceListingError,
  PinterestShopConnector, PinterestShopListingError
};
