...
// ─────────────────────────────────────────────────────────────
// Bonanza — REST API v2, OAuth 2.0 access token. Docs: bonanza.com/api_ref
//
// Not per-tenant, single shared BONANZA_ACCESS_TOKEN, matching how
// lib/bonanza_listing.py's own _get_token() reads it — same shape as
// FacebookConnector, not the per-tenant vault pattern eBay/Etsy use.
//
// createListing() does NOT reimplement Bonanza's API here — that logic
// lives in lib/bonanza_listing.py (canonical client, 19 tests), called
// through the same internal Python bridge service used for eBay/Etsy/
// Facebook (scripts/listing_service.py, localhost only,
// /bonanza/create-listing).
// ─────────────────────────────────────────────────────────────
class BonanzaConnector extends BaseConnector {
  static ENV = ["BONANZA_ACCESS_TOKEN"];
  ...
  async getConnectionStatus() {
    if (!this.hasRequiredEnv(BonanzaConnector.ENV)) {
      return {
        ...
        detail: "Optional — not configured. Needs a Bonanza seller account + BONANZA_ACCESS_TOKEN (bonanza.com/api_ref).",
      };
    }
    return { status: CONNECTION_STATUS.CONFIG_REQUIRED, detail: "Token present — run Test Connection to verify (never assumed connected without a live test)." };
  }

  /** Live check: lists existing listings (a harmless read) to prove the
   * token actually authenticates, not just that it's present. */
  async testConnection() {
    if (!this.hasRequiredEnv(BonanzaConnector.ENV)) {
      return { status: CONNECTION_STATUS.NOT_CONNECTED, detail: "BONANZA_ACCESS_TOKEN is not set." };
    }
    try {
      const res = await fetch("https://api.bonanza.com/api_ref/listings", {
        headers: { Authorization: `Bearer ${process.env.BONANZA_ACCESS_TOKEN}` },
        signal: AbortSignal.timeout(10_000),
      });
      return res.ok
        ? { status: CONNECTION_STATUS.CONNECTED, detail: "Bonanza API call succeeded" }
        : { status: CONNECTION_STATUS.CONFIG_REQUIRED, detail: `Bonanza auth failed (HTTP ${res.status})` };
    } catch (err) {
      return {
        status: CONNECTION_STATUS.CONFIG_REQUIRED,
   ...
   * API; it only forwards the request with the shared access token.
   *
   * @param {object} listing Matches BonanzaListing fields — see
   *   lib/bonanza_listing.py: title, description, price, quantity, sku,
   *   category_id, condition, tags, image_urls, shipping_weight_lbs,
   *   returns_accepted.
   * @param {object} [options]
   * @param {boolean} [options.dryRun=true]
   * @param {string} [options.confirm] Must be exactly "PUBLISH_LIVE" for a
  ...
  async createListing(listing, options = {}) {
    ...
    if (!dryRun && !this.hasRequiredEnv(BonanzaConnector.ENV)) {
      throw new BonanzaListingError(
        ...
        "BONANZA_ACCESS_TOKEN must be set before creating a live listing.",
        { statusCode: 409 },
      );
    }
