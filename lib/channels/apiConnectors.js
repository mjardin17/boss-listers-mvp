...
class EbayConnector extends BaseConnector {
  ...
  async createListing(product, policies, options = {}) {
    const { dryRun = true, confirm, sandbox = false, tenantId, serviceToken } = options;
    ...
    if (dryRun === false) {
      ...
      // options.serviceToken lets a caller supply this per-request (e.g. a
      // one-off admin call) without requiring EBAY_LISTING_SERVICE_TOKEN to
      // be set in this process's own environment / restarting the server.
      const token = serviceToken || process.env.EBAY_LISTING_SERVICE_TOKEN;
      if (token) headers["X-Listing-Service-Token"] = token;
    }
...
class InstagramConnector extends BaseConnector {
  ...
  static LISTING_SERVICE_URL = process.env.INSTAGRAM_LISTING_SERVICE_URL || process.env.FACEBOOK_LISTING_SERVICE_URL || process.env.EBAY_LISTING_SERVICE_URL || "http://127.0.0.1:8791";
  ...
  async createListing(product, options = {}) {
    ...
    if (!dryRun && !this.hasRequiredEnv(InstagramConnector.ENV)) {
      throw new InstagramListingError(
        "instagram_not_configured",
        "IG_ACCESS_TOKEN and IG_APP_ID must both be set before creating a live listing.",
        { statusCode: 409 },
      );
    }

    const requestBody = {
      access_token: process.env.IG_ACCESS_TOKEN || "dry-run-placeholder",
      app_id: process.env.IG_APP_ID || "dry-run-placeholder",
      product,
      dry_run: dryRun,
    };
    if (confirm !== undefined) requestBody.confirm = confirm;
    ...
    const headers = { "Content-Type": "application/json" };
    if (dryRun === false) {
      const serviceToken = process.env.INSTAGRAM_LISTING_SERVICE_TOKEN || process.env.FACEBOOK_LISTING_SERVICE_TOKEN || process.env.EBAY_LISTING_SERVICE_TOKEN;
      if (serviceToken) headers["X-Listing-Service-Token"] = serviceToken;
    }

    let res;
    try {
      res = await fetch(`${InstagramConnector.LISTING_SERVICE_URL}/instagram/create-listing`, {
        method: "POST", headers, body: JSON.stringify(requestBody),
        signal: AbortSignal.timeout(30_000),
      });
    } catch (err) {
      throw new InstagramListingError(
        "bridge_unreachable",
        err.name === "TimeoutError" ? "Instagram listing service did not respond within 30s" : `Instagram listing service unreachable at ${InstagramConnector.LISTING_SERVICE_URL} — is it running? (${err.message})`,
        { statusCode: 503 },
      );
    }
    ...
    if (!body || typeof body !== "object") {
      throw new InstagramListingError("bridge_bad_response", `Instagram listing service returned a non-JSON response (HTTP ${res.status})`, { statusCode: 502 });
    }
    ...
    if (!res.ok) {
      ...
      throw new InstagramListingError(
        (detail && detail.code) || "listing_failed",
        (detail && detail.message) || `Instagram listing service returned HTTP ${res.status}`,
        { statusCode: res.status, step: detail && detail.step, instagramStatus: detail && detail.instagram_status },
      );
    }
...
class AmazonConnector extends BaseConnector {
  ...
  /** Not implemented — see class comment. Throws rather than silently
   * pretending to publish, so callers fail loudly instead of getting a
   * false "success". */
  async createListing() {
    throw new AmazonListingError(
      "not_implemented",
      "Amazon listing creation isn't built yet — SP-API's Listings Items API requires AWS SigV4-signed requests to a region-specific endpoint, which is separate work from the OAuth connection this class handles.",
      { statusCode: 501 },
    );
  }
}
...
class TikTokShopConnector extends BaseConnector {
  ...
  async createListing() {
    throw new TikTokShopListingError(
      "not_implemented",
      "TikTok Shop listing creation isn't built yet — its Product API requires a separate HMAC-SHA256 request-signing scheme, which is separate work from the OAuth connection this class handles.",
      { statusCode: 501 },
    );
  }
}
 ...
 *
 * createListing() is NOT implemented — Shopify handles listing creation
 * server-side via the storefront, and this connector's role is inventory
 * sync and order tracking. Throws "not_implemented" rather than pretending.
 */
class ShopifyConnector extends BaseConnector {
  ...
  async testConnection() {
    if (!this.hasRequiredEnv(ShopifyConnector.ENV)) {
      return { status: CONNECTION_STATUS.CONFIG_REQUIRED, detail: "SHOPIFY_STORE_URL, SHOPIFY_CLIENT_ID, and SHOPIFY_CLIENT_SECRET are all required." };
    }
  ...
  async _getAccessToken(tenantId) {
    ...
    // Shopify access tokens don't expire — once issued, they're valid until
    // the custom app is deleted or the token is regenerated. Cache indefinitely.
    const cached = { token: conn.accessToken, expiresAt: now + (365 * 24 * 60 * 60 * 1000) };
    this._accessTokenCache[cacheKey] = cached;
    return cached;
  }
  ...
  async createListing() {
    throw new ShopifyListingError(
      "not_implemented",
      "Shopify listing creation isn't built yet — Shopify handles product management server-side via the admin dashboard/API, which is separate from this connector's role (inventory sync and order tracking). A future Shopify listing bridge would build products via the REST API.",
      { statusCode: 501 },
    );
  }
}

module.exports = { EbayConnector, EbayListingError, EtsyConnector, EtsyListingError, FacebookConnector, FacebookListingError, InstagramConnector, InstagramListingError, BonanzaConnector, BonanzaListingError, ShopifyConnector, ShopifyListingError, WooCommerceConnector, WooCommerceListingError, AmazonConnector, AmazonListingError, TikTokShopConnector, TikTokShopListingError };
