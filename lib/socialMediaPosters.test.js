...
const MOCK_RESPONSES = {
  ...
  /**
   * Pinterest API v5 Response
   * Endpoints: POST /v5/media/uploads, GET /v5/media/uploads/{id}, POST /v5/pins
   */
  pinterest: {
    registerMedia: {
      media_id: "1234567890",
...
describe("Pinterest Poster", () => {
  test("Pinterest pin creation with video", async () => {
    /**
     * Expected v5 3-Step Sequence:
     * 1. POST https://api.pinterest.com/v5/media/uploads with { media_type: "video/mp4" }
     * 2. POST <upload_url> with FormData containing S3 parameters + video binary
     * 3. GET https://api.pinterest.com/v5/media/uploads/{id} returning { status: "succeeded" }
     * 4. POST https://api.pinterest.com/v5/pins with board_id, title, description, media_source: { source_type: "video_id", media_id }
     */

    global.fetch = jest.fn((url, options) => {
      const urlStr = String(url);
      if (urlStr.includes("/v5/media/uploads/")) {
        return Promise.resolve({
          ok: true,
          status: 200,
          json: () => Promise.resolve(MOCK_RESPONSES.pinterest.mediaStatus),
          text: () => Promise.resolve(JSON.stringify(MOCK_RESPONSES.pinterest.mediaStatus)),
        });
      }
      if (urlStr.includes("/v5/media/uploads")) {
        return Promise.resolve({
          ok: true,
          status: 201,
          json: () => Promise.resolve(MOCK_RESPONSES.pinterest.registerMedia),
          text: () => Promise.resolve(JSON.stringify(MOCK_RESPONSES.pinterest.registerMedia)),
        });
      }
      if (urlStr.includes("amazonaws.com") || urlStr.includes("upload")) {
        return Promise.resolve({
          ok: true,
          status: 204,
          text: () => Promise.resolve(""),
      ...
      if (urlStr.includes("/v5/pins")) {
        return Promise.resolve({
          ok: true,
          status: 201,
          json: () => Promise.resolve(MOCK_RESPONSES.pinterest.success),
          text: () => Promise.resolve(JSON.stringify(MOCK_RESPONSES.pinterest.success)),
        });
      }
      return Promise.resolve({
        ok: true,
