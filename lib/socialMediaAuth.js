...
const OAUTH_CONFIGS = {
  ...
  ebay: {
    ...
    redirectUri: `${process.env.APP_URL}/api/oauth/ebay/callback`,
...
async function getAccessToken(platform, code) {
  ...
  // eBay requires Basic Auth for token endpoint
  if (platform === 'ebay') {
  ...
  // Non-eBay platforms include client credentials in body
  if (platform !== 'ebay') {
