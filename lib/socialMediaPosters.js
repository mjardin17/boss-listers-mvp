...
const POSTERS = {
  ...
  /**
   * Pinterest Pin with Video (API v5)
   *
   * 3-Step Asynchronous Flow:
   * 1. Register media upload: POST /v5/media/uploads with { media_type: "video/mp4" }
   *    -> returns media_id, upload_url, upload_parameters
   * 2. Upload video binary bytes to upload_url with upload_parameters
   *    -> verify upload success (HTTP 200/204)
   * 3. Check media processing status via GET /v5/media/uploads/{media_id}:
   *    - If "succeeded": POST /v5/pins with board_id, title, description, media_source
   *    - If "processing": surface clear "processing" state rather than failing
   *    - If "failed": throw descriptive error
   *
   * Exact signature preserved:
   * pinterest(videoBuffer, caption, accessToken, boardId = null)
   *
   * Return shape preserved:
   * { platform, pinId, url, boardId, timestamp, [status] }
   */
  async pinterest(videoBuffer, caption, accessToken, boardId = null) {
    ...
    try {
      ...
      log(platform, "info", "Starting Pinterest v5 video pin creation", {
        videoSize: (videoBuffer.length / 1024 / 1024).toFixed(2),
        boardId,
      });

      const executePostFn = async () => {
        ...
        log(platform, "info", "Registering media upload with Pinterest API v5");
        const registerResponse = await fetchWithTimeout(
          `${config.baseUrl}/${config.apiVersion}/media/uploads`,
          {
            method: "POST",
            headers: authHeaders,
            body: JSON.stringify({ media_type: "video/mp4" }),
          },
          config.timeout
        );

        if (!registerResponse.ok) {
          const errorMsg = await parsePlatformError(registerResponse, platform);
          throw new Error(`Pinterest media registration rejected: ${errorMsg}`);
        }
        ...
        if (!mediaId || !uploadUrl) {
          throw new Error(
            "Pinterest media registration rejected: Missing media_id or upload_url in response"
          );
        }

        log(platform, "info", "Media upload registered", { mediaId, uploadUrl });

        // ── STEP 2: Upload Video Bytes to Upload URL ───────────────────────────
        log(platform, "info", "Uploading video bytes to upload URL");
        const uploadFormData = new FormData();

        // Append all parameters returned by Pinterest (e.g. S3 presigned form fields)
        if (typeof uploadParameters === "object" && uploadParameters !== null) {
          for (const [key, value] of Object.entries(uploadParameters)) {
            uploadFormData.append(key, value);
          }
        }

        // Append video binary blob as "file"
        ...
        try {
          ...
          throw new Error(`Pinterest video upload failed: ${uploadErr.message}`);
        }

        if (!uploadResponse.ok) {
          const uploadErrorText = await uploadResponse.text().catch(() => "");
          throw new Error(
            `Pinterest video upload failed: HTTP ${uploadResponse.status} ${uploadResponse.statusText}${uploadErrorText ? ` - ${uploadErrorText}` : ""}`
          );
        }

        log(platform, "info", "Video upload to storage succeeded", { mediaId });

        // ── STEP 2b: Check Media Processing Status ─────────────────────────────
        log(platform, "info", "Checking media processing status", { mediaId });
        let statusData = { status: "succeeded" };
        try {
          const statusResponse = await fetchWithTimeout(
            `${config.baseUrl}/${config.apiVersion}/media/uploads/${mediaId}`,
            {
              method: "GET",
              headers: {
                Authorization: `Bearer ${accessToken}`,
          ...
          if (statusResponse.ok) {
            ...
            log(platform, "warn", "Media status check returned non-200, proceeding with pin creation", {
              status: statusResponse.status,
            });
          }
        } catch (statusErr) {
          log(platform, "warn", "Media status check error, proceeding with pin creation", {
            error: statusErr.message,
          });
        }

        const mediaStatus = statusData?.status?.toLowerCase() || "succeeded";

        if (mediaStatus === "failed") {
          throw new Error(
            `Pinterest media processing failed: ${statusData.failure_code || statusData.error || "processing failed"}`
          );
        }

        // If media is still in processing state, surface a clear processing state rather than failing
        if (mediaStatus === "processing" || mediaStatus === "registered") {
          log(platform, "info", "Pinterest media is still processing", { mediaId, mediaStatus });
          return {
            platform,
            pinId: null,
            url: null,
            ...
            mediaId,
            status: "processing",
            message: "Pinterest video is currently processing. Pin will be created once processing completes.",
            timestamp: new Date().toISOString(),
          };
        }

        // ── STEP 3: Create Pin (mediaStatus === "succeeded") ───────────────────
        const title = sanitizedCaption.split("\n")[0].slice(0, 100);
        const description = sanitizedCaption;
        ...
        log(platform, "info", "Creating pin with media_id", { mediaId, boardId, title });
        ...
        if (!pinResponse.ok) {
          const errorMsg = await parsePlatformError(pinResponse, platform);
          throw new Error(`Pinterest pin creation rejected: ${errorMsg}`);
        }
      ...
      return await retryWithBackoff(executePostFn, config.maxRetries, config.retryDelay, platform);
    } catch (err) {
      log(platform, "error", err.message, { stack: err.stack });
      throw err;
    }
